...
export async function up(queryInterface: QueryInterface): Promise<void> {
  await queryInterface.sequelize.query(
    `
CREATE VIEW collated_composer_albums_data AS
SELECT 
	albums.id,
	albums.title,
	albums.year,
	albums.account_id AS account_id,
...
INNER JOIN linked_composers ON linked_composers.file_id = files.id
INNER JOIN composers ON composers.id = linked_composers.composer_id
GROUP BY albums.id
    `,
  );
}
...
export async function down(queryInterface: QueryInterface): Promise<void> {
  await queryInterface.sequelize.query(
    `
      DROP VIEW IF EXISTS collated_composer_albums_data;
    `,
  );
}
