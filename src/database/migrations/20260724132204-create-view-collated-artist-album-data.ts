...
export async function up(queryInterface: QueryInterface): Promise<void> {
  await queryInterface.sequelize.query(
    `
CREATE VIEW collated_artist_albums_data AS
SELECT 
	albums.id,
	albums.title,
	albums.year,
...
FROM albums 
INNER JOIN album_artists ON album_artists.album_id = albums.id
INNER JOIN artists ON artists.id = album_artists.artist_id
    `,
  );
}
...
export async function down(queryInterface: QueryInterface): Promise<void> {
  await queryInterface.sequelize.query(
    `
      DROP VIEW IF EXISTS collated_artist_albums_data;
    `,
  );
}
