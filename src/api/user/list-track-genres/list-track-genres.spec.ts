...
describe('/users/list-track-genres', () => {
  let userApi: UserApi;
  ...
  describe('authorized access', () => {
    it('should reject guest access', async () => {
      const { error } = await api.GET(`/api/user/list-track-genres`, {
        params: {
          header: {
            Authorization: '',
          },
  ...
  describe('errors', () => {
    it('should reject negative limit', async () => {
      const { error } = await userApi.listTrackGenres({ offset: 0, limit: -1000 });
      expect(error?.message[0]).toBe(ErrorCodes.INVALID_LIMIT_ERROR);
    });

    it('should reject excessive "limit"', async () => {
      const { error } = await userApi.listTrackGenres({ offset: 0, limit: 1_000_000 });
      expect(error?.message[0]).toBe(ErrorCodes.INVALID_LIMIT_ERROR);
    });

    it('should reject invalid limit', async () => {
      const { error } = await userApi.listTrackGenres({ offset: 0, limit: 'asdf' as unknown as number });
      expect(error?.message[0]).toBe(ErrorCodes.INVALID_LIMIT_ERROR);
    });

    it('should reject negative offset', async () => {
      const { error } = await userApi.listTrackGenres({ offset: -1000 });
      expect(error?.message[0]).toBe(ErrorCodes.INVALID_OFFSET_ERROR);
    });

    it('should reject invalid offset', async () => {
      const { error } = await userApi.listTrackGenres({ offset: 'asdf' as unknown as number });
      expect(error?.message[0]).toBe(ErrorCodes.INVALID_OFFSET_ERROR);
    });

    it('should reject invalid sortDirection', async () => {
      const { error } = await userApi.listTrackGenres({
  ...
  describe('edge cases', () => {
    describe('sort', () => {
      it('should sort by genre ASC', async () => {
        const { data } = await userApi.listTrackGenres({
  ...
  describe('success', () => {
    ...
    it('should paginate results', async () => {
      const { data } = await userApi.listTrackGenres({ offset: 0, limit: 4 });
      const { genres, total } = data || { genres: [], total: 0 };
      expect(total).toBe(7);
      ...
      expect(genres[2]?.name).toBe('Acoustic');
      expect(genres[3]?.name).toBe('Bebob');
      const { data: data2 } = await userApi.listTrackGenres({ offset: 4, limit: 4 });
