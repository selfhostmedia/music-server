...
describe('/users/list-track-artists-with-tracks', () => {
  let userApi: UserApi;
  ...
  describe('authorized access', () => {
    it('should reject guest access', async () => {
      const { error } = await api.GET(`/api/user/list-track-artists-with-tracks`, {
        params: {
          header: {
            Authorization: '',
          },
  ...
  describe('errors', () => {
    ...
    it('should reject invalid genre length', async () => {
      const { error } = await userApi.listTrackArtistsWithTracks({ genre: ['x'.repeat(300)] });
      expect(error?.message[0]).toBe(ErrorCodes.INVALID_GENRE_LENGTH_ERROR);
    });

    it('should reject negative limit', async () => {
      const { error } = await userApi.listTrackArtistsWithTracks({ offset: 0, limit: -1000 });
      expect(error?.message[0]).toBe(ErrorCodes.INVALID_LIMIT_ERROR);
    });

    it('should reject excessive "limit"', async () => {
      const { error } = await userApi.listTrackArtistsWithTracks({ offset: 0, limit: 1_000_000 });
      expect(error?.message[0]).toBe(ErrorCodes.INVALID_LIMIT_ERROR);
    });

    it('should reject invalid limit', async () => {
      const { error } = await userApi.listTrackArtistsWithTracks({ offset: 0, limit: 'asdf' as unknown as number });
      expect(error?.message[0]).toBe(ErrorCodes.INVALID_LIMIT_ERROR);
    });

    it('should reject negative offset', async () => {
      const { error } = await userApi.listTrackArtistsWithTracks({ offset: -1000 });
      expect(error?.message[0]).toBe(ErrorCodes.INVALID_OFFSET_ERROR);
    });

    it('should reject invalid offset', async () => {
      const { error } = await userApi.listTrackArtistsWithTracks({ offset: 'asdf' as unknown as number });
      expect(error?.message[0]).toBe(ErrorCodes.INVALID_OFFSET_ERROR);
    });

    it('should reject invalid sortDirection', async () => {
      const { error } = await userApi.listTrackArtistsWithTracks({
  ...
  describe('edge cases', () => {
    describe('filter', () => {
      it('should filter by genre', async () => {
        const { data } = await userApi.listTrackArtistsWithTracks({ genre: ['Rock'] });
      ...
      it('should filter by search term', async () => {
        const { data } = await userApi.listTrackArtistsWithTracks({ filter: '3' });
        const { artists, total } = data || { artists: [], total: 0 };
        expect(total).toBe(3);
  ...
  describe('success', () => {
    ...
    it('should paginate results', async () => {
      const { data } = await userApi.listTrackArtistsWithTracks({ offset: 0, limit: 2 });
      const { artists, total } = data || { artists: [], total: 0 };
      expect(total).toBe(5);
      expect(artists.length).toBe(2);
      expect(artists[0]?.name).toBe('Artist 1');
      expect(artists[1]?.name).toBe('Artist 2');
      const { data: data2 } = await userApi.listTrackArtistsWithTracks({ offset: 1, limit: 2 });
      ...
      const { data: data3 } = await userApi.listTrackArtistsWithTracks({ offset: 2, limit: 2 });
