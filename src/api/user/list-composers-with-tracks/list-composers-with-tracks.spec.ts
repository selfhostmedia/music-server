...
describe('/users/list-composers-with-tracks', () => {
  let userApi: UserApi;
  ...
  describe('authorized access', () => {
    it('should reject guest access', async () => {
      const { error } = await api.GET(`/api/user/list-composers-with-tracks`, {
        params: {
          header: {
            Authorization: '',
          },
  ...
  describe('errors', () => {
    it('should reject invalid addedAfter date', async () => {
      const { error } = await userApi.listComposersWithTracks({ addedAfter: 'invalid-date' });
      expect(error?.message[0]).toBe(ErrorCodes.INVALID_ADDED_AFTER_ERROR);
    });

    it('should reject invalid addedBefore date', async () => {
      const { error } = await userApi.listComposersWithTracks({ addedBefore: 'invalid-date' });
      expect(error?.message[0]).toBe(ErrorCodes.INVALID_ADDED_BEFORE_ERROR);
    });

    it('should reject invalid filter', async () => {
      const { error } = await userApi.listComposersWithTracks({ filter: '' });
      expect(error?.message[0]).toBe(ErrorCodes.INVALID_FILTER_LENGTH_ERROR);
    });

    // it('should reject invalid genre', async () => {
    //   const { error } = await userApi.listComposersWithTracks({ genre: [0 as unknown as string] });
    //   expect(error?.message[0]).toBe(ErrorCodes.INVALID_GENRE_LENGTH_ERROR);
    // });

    it('should reject invalid genre length', async () => {
      const { error } = await userApi.listComposersWithTracks({ genre: ['x'.repeat(300)] });
      expect(error?.message[0]).toBe(ErrorCodes.INVALID_GENRE_LENGTH_ERROR);
    });

    it('should reject negative limit', async () => {
      const { error } = await userApi.listComposersWithTracks({ offset: 0, limit: -1000 });
      expect(error?.message[0]).toBe(ErrorCodes.INVALID_LIMIT_ERROR);
    });

    it('should reject excessive "limit"', async () => {
      const { error } = await userApi.listComposersWithTracks({ offset: 0, limit: 1_000_000 });
      expect(error?.message[0]).toBe(ErrorCodes.INVALID_LIMIT_ERROR);
    });

    it('should reject invalid limit', async () => {
      const { error } = await userApi.listComposersWithTracks({ offset: 0, limit: 'asdf' as unknown as number });
      expect(error?.message[0]).toBe(ErrorCodes.INVALID_LIMIT_ERROR);
    });

    it('should reject negative offset', async () => {
      const { error } = await userApi.listComposersWithTracks({ offset: -1000 });
      expect(error?.message[0]).toBe(ErrorCodes.INVALID_OFFSET_ERROR);
    });

    it('should reject invalid offset', async () => {
      const { error } = await userApi.listComposersWithTracks({ offset: 'asdf' as unknown as number });
      expect(error?.message[0]).toBe(ErrorCodes.INVALID_OFFSET_ERROR);
    });

    it('should reject invalid sortDirection', async () => {
      const { error } = await userApi.listComposersWithTracks({
  ...
  describe('edge cases', () => {
    describe('filter', () => {
      it('should filter by genre', async () => {
        const { data } = await userApi.listComposersWithTracks({ genre: ['Chanson'] });
      ...
      it('should filter by search term', async () => {
        const { data } = await userApi.listComposersWithTracks({ filter: '3' });
        const { composers, total } = data || { composers: [], total: 0 };
        expect(total).toBe(2);
    ...
    describe('sort', () => {
      it('should sort by composer ASC', async () => {
        const { data } = await userApi.listComposersWithTracks({
  ...
  describe('success', () => {
    ...
    it('should paginate results', async () => {
      const { data } = await userApi.listComposersWithTracks({ offset: 0, limit: 4 });
      const { composers, total } = data || { composers: [], total: 0 };
      expect(total).toBe(10);
      ...
      expect(composers[2]?.name).toBe('Artist 3');
      expect(composers[3]?.name).toBe('Composer 1');
      const { data: data2 } = await userApi.listComposersWithTracks({ offset: 4, limit: 4 });
      ...
      const { data: data3 } = await userApi.listComposersWithTracks({ offset: 8, limit: 4 });
