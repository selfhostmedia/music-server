...
describe('/users/list-album-artists-with-tracks', () => {
  let userApi: UserApi;
  ...
  describe('errors', () => {
    it('should reject invalid addedAfter date', async () => {
      const { error } = await userApi.listAlbumArtistsWithTracks({ addedAfter: 'invalid-date' });
      expect(error?.message[0]).toBe(ErrorCodes.INVALID_ADDED_AFTER_ERROR);
    });

    it('should reject invalid addedBefore date', async () => {
      const { error } = await userApi.listAlbumArtistsWithTracks({ addedBefore: 'invalid-date' });
      expect(error?.message[0]).toBe(ErrorCodes.INVALID_ADDED_BEFORE_ERROR);
    });

    it('should reject invalid filter', async () => {
      const { error } = await userApi.listAlbumArtistsWithTracks({ filter: '' });
      expect(error?.message[0]).toBe(ErrorCodes.INVALID_FILTER_LENGTH_ERROR);
    });

    // it('should reject invalid genre', async () => {
    //   const { error } = await userApi.listAlbumArtistsWithTracks({ genre: [0 as unknown as string] });
    //   expect(error?.message[0]).toBe(ErrorCodes.INVALID_GENRE_LENGTH_ERROR);
    // });

    it('should reject invalid genre length', async () => {
      const { error } = await userApi.listAlbumArtistsWithTracks({ genre: ['x'.repeat(300)] });
      expect(error?.message[0]).toBe(ErrorCodes.INVALID_GENRE_LENGTH_ERROR);
    });

    it('should reject negative limit', async () => {
      const { error } = await userApi.listAlbumArtistsWithTracks({ offset: 0, limit: -1000 });
      expect(error?.message[0]).toBe(ErrorCodes.INVALID_LIMIT_ERROR);
    });

    it('should reject excessive "limit"', async () => {
      const { error } = await userApi.listAlbumArtistsWithTracks({ offset: 0, limit: 1_000_000 });
      expect(error?.message[0]).toBe(ErrorCodes.INVALID_LIMIT_ERROR);
    });

    it('should reject invalid limit', async () => {
      const { error } = await userApi.listAlbumArtistsWithTracks({ offset: 0, limit: 'asdf' as unknown as number });
      expect(error?.message[0]).toBe(ErrorCodes.INVALID_LIMIT_ERROR);
    });

    it('should reject negative offset', async () => {
      const { error } = await userApi.listAlbumArtistsWithTracks({ offset: -1000 });
      expect(error?.message[0]).toBe(ErrorCodes.INVALID_OFFSET_ERROR);
    });

    it('should reject invalid offset', async () => {
      const { error } = await userApi.listAlbumArtistsWithTracks({ offset: 'asdf' as unknown as number });
      expect(error?.message[0]).toBe(ErrorCodes.INVALID_OFFSET_ERROR);
    });

    it('should reject invalid sortDirection', async () => {
      const { error } = await userApi.listAlbumArtistsWithTracks({
  ...
  describe('edge cases', () => {
    describe('filter', () => {
      it('should filter by genre', async () => {
        const { data } = await userApi.listAlbumArtistsWithTracks({ genre: ['Rock'] });
      ...
      it('should filter by search term', async () => {
        const { data } = await userApi.listAlbumArtistsWithTracks({ filter: '3' });
        const { artists, total } = data || { artists: [], total: 0 };
        expect(total).toBe(1);
    ...
    describe('sort', () => {
      it('should sort by artist ASC', async () => {
        const { data } = await userApi.listAlbumArtistsWithTracks({
  ...
  describe('success', () => {
    ...
    it('should paginate results', async () => {
      const { data } = await userApi.listAlbumArtistsWithTracks({ offset: 0, limit: 2 });
      const { artists, total } = data || { artists: [], total: 0 };
      expect(total).toBe(3);
      expect(artists.length).toBe(2);
      expect(artists[0]?.name).toBe('Artist 1');
      expect(artists[1]?.name).toBe('Artist 2');
      const { data: data2 } = await userApi.listAlbumArtistsWithTracks({ offset: 1, limit: 2 });
      ...
      const { data: data3 } = await userApi.listAlbumArtistsWithTracks({ offset: 2, limit: 2 });
