...
function configurationToRow(
  config: IndexerConfigurationEntity,
...
export class AdminIndexerConfigurationService {
  ...
  async getIndexerConfiguration(): Promise<AdminIndexerConfigurationDto> {
    ...
    const configuration = await this.indexerConfigurationModel.findOne({
      attributes: ['isEnabled', 'createdAt', 'createdByAccountId'],
      include: [
        {
          model: AccountEntity,
          attributes: ['username'],
    ...
    if (!configuration) {
      throw new InternalServerErrorException(
        ErrorCodes.INDEXER_CONFIGURATION_NOT_FOUND_ERROR,
      );
    }
